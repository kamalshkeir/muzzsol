...
// Encrypt is aes encrypt, it use secret from .env
func Encrypt(data string) (string, error) {
	keyEnv = settings.Config.Secret
	if keyEnv == "" {
		keyEnv = kstring.Random(19)
		settings.Config.Secret = keyEnv
	}
...
// Decrypt is aes decrypt, it use secret from .env
